    ...
    <div className="fixed top-0 h-[50px] sm:h-[120px] w-full z-10 bg-white drop-shadow-lg">
      <nav className="flex flex-row items-center w-full h-full px-8 sm:px-16 lg:px-24 ">
        ...
        <ul className="hidden lg:flex flex-row justify-end items-center gap-12 w-4/5">
          {navigation.map((item,index)=>(
            <a key={"Link_"+index} 
              href={item.href}
            >
              <li key={"Link_"+item.name} className="mx-4 text-xl  font-body text-fourth hover:text-primary hover:underline ease-in-out duration-300">{item.label}</li>
            </a>
          ))}
          <div className="hidden lg:flex flex-row gap-4">
            <Link size="sm" className="px-[24px]" href="#contact" variant="ghost"><User/>Mi Olimed</Link>
        ...
        <div className="flex justify-end lg:hidden w-auto gap-4">
          <a className="text-primary hover:text-tertiary hover:underline" href="#contact"><User className="h-6 w-6 sm:h-10 sm:w-10"/></a>
