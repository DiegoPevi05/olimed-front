    ...
    <div className="w-full h-full mt-[50px] sm:mt-[120px] py-4">
      ...
      <div className="mx-4 mx-auto flex flex-col lg:flex-row max-w-2xl items-start px-4 sm:px-6 lg:max-w-7xl lg:px-8 py-4 mt-10 lg:py-8 w-full">
        <div className="flex flex-col justify-start w-full lg:w-1/2">
          <h2 className={styles.sectionNewsSubHeader}>{t(news.subHeader)}</h2>
          <h1 className={styles.sectionNewsHeader}>{t(news.header)}</h1>
          <p className={styles.sectionNewsBody}>{t(news.body)}</p>
        </div>
        <div className="mx-4 p-4 flex flex-col flex-start items-center w-full lg:w-1/2 ">
            ...
            <div className="flex flex-start w-full h-auto">
              <Link size="sm" href={news.external_news} target="_blank" className="px-4 lg:ml-4 mt-10" variant="ghost2">{t("Ir a Sitio Web")} <Globe2 className="h-4 w-4 ml-2"/></Link>
            </div>
